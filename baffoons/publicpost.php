...
<li class="active"><a href="publicpost.php"><span class='glyphicon glyphicon-cloud'></span> Public Board</a></li>
<li><a href="friendboard.php"><span class='glyphicon glyphicon-th-list'></span> Friends Board</a></li>
</ul>		
</div>
</div>
...
<form action="bulletin.php" method="POST" class="form-group"><label for=" ">Post Message:</label><textarea class="form-control" name="msgpost"></textarea><button class="btn btn-success pull-right" type="submit" name="post" id="postbutton">Post</button></form>
</div>

<div class="post-image-box">
<form action="postpublicphotos.php" method='POST' enctype='multipart/form-data' class="form-group"><label for=" ">Post an Image:</label><input type='file' name='myfile' id='myfile' accept="image/*" /><button class="btn btn-success pull-right" type='submit' name='upload'>Post</button></form>
</div>

<div class="post-video-box">
<form action="bulletin.php" method="POST" class="form-group"><label for=" ">Post Youtube Link:</label><textarea class="form-control" name="vdopost"></textarea><button class="btn btn-success pull-right" type="submit" name="vidpost" id="postbutton">Post</button></form>
</div>
...
<?php
$perpage = 20;
$msg = $pdo->prepare("SELECT * FROM bulletin");
$msg->execute();
$msgcount = $msg->rowCount();
$pages = ceil(($msgcount) / $perpage);
$page = (isset($_GET['page'])) ? (int)$_GET['page'] : 1;
$start = ($page - 1) * $perpage;
$msgresult = $pdo->prepare("SELECT * FROM bulletin ORDER BY Time DESC LIMIT $start, $perpage");
$msgresult->execute();
...
$postud->execute(array('UserID'=>$postuid));
while($pudrow = $postud->fetch()){
$pudfname = $pudrow['Firstname'];
$pudlname = $pudrow['Lastname'];
...
echo "<button class='close pull-right' type='submit' name='delete' id='postdelete'>&times;</button>";
echo "</form>";
}
echo "<h5>";
echo $pudfname." ".$pudlname;
echo "</h5>";
echo "<p>".$postm."</p>";
echo "<div class='label label-info pull-left'>$posttime</div>";
if($postuid != $suid){
echo "<form action='report.php' method='POST'>";
echo "<input type='hidden' name='repuid' value='$postuid' />";
echo "<input type='hidden' name='repmsg' value='$postm' />";
echo "<input type='hidden' name='reploc' value='bulletin' />";
echo "<button class='btn btn-warning btn-xs pull-right' type='submit' name='report' id='postdelete'>Report</button>";
echo "</form>";
}
echo "</div></div>";
}
