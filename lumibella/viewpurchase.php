...
$selectorder = $pdo->prepare("SELECT * FROM orders WHERE orderid=:vpid");
...
echo "<tr><td>Payment Method:</td><td>CARD / ONLINE PAYMENT / CASH ON DELIVERY</td></tr>";
...
$getod = $pdo->prepare("SELECT * FROM orderdetail WHERE orderid=:vpid");
$getod->execute(array(
				"vpid"=>$vpid
				));
echo "<table class='table cart-table'>";
...
?>
<a class="btn btn-primary btn-lg pull-right" href="purchases.php">Purchase List</a>
</div>
</div>
