...
<hr>
<?php
if(isset($_REQUEST['dvoid'])){
$oid = $_REQUEST['dvoid'];
//set delivered
$delivered = 1;
$deliveredorder = $pdo->prepare("UPDATE orders SET status=:delivered WHERE orderid=:oid");
$deliveredorder->execute(array(
					"delivered"=>$delivered,
					"oid"=>$oid
					));
if($deliveredorder == true){
echo "<div class='alert alert-info'>This order has been set to status as delivered.</div>";
}
else{
echo "<div class='alert alert-info'>Error occurred. Unable to set this order status as delivered</div>";
}
...
echo "</table>";
echo "<a class='btn btn-primary btn-lg pull-right shipping-button' href='orderlist.php'>Orders List</a>";
}
}
?>
</div>
