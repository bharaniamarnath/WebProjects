...
<div id="holder">
<ul>
<li><a id="onlink" href="groups.php">My Groups</a></li>
...
<?php 
$perpage = 20;
$msg = $pdo->prepare("SELECT * FROM groupmembers WHERE UserID=:UserID");
$msg->execute(array('UserID'=>$suid));
$msgcount = $msg->rowCount();
$pages = ceil(($msgcount) / $perpage);
$page = (isset($_GET['page'])) ? (int)$_GET['page'] : 1;
$start = ($page - 1) * $perpage;
$gmember = $pdo->prepare("SELECT * FROM groupmembers WHERE UserID=:UserID");
$gmember->execute(array('UserID'=>$suid));
if($gmember->rowCount()==0){
echo $emptygrpalert;
}
while($gmem = $gmember->fetch()){
$gid = $gmem['ID'];
$groups = $pdo->prepare("SELECT * FROM groups WHERE ID=:ID ORDER BY Date DESC LIMIT $start, $perpage");
$groups->execute(array('ID'=>$gid));
...
echo "<img align='left' id='thumbimgcrop' src='$groupimagethumb'><p>$groupname<br /><font id='postmessage'>$grouptype</font></p><a href='viewgroup.php?vgrpid=$groupid'>View</a></div>";
}
}
?>
</div>
