...
<li><a href="albumfriend.php?faid=<?php echo $fproaid; ?>">Photos</a></li>
</ul>		
</div>
</div>
...
$fprores->execute(array('UserID'=>$fpro));
while($fprow = $fprores->fetch()){
$userid = $fprow['UserID'];
...
$fproresult = $pdo->prepare("SELECT * FROM personaldetails WHERE Username=:Username");
$fproresult->execute(array('Username'=>$fpro));
...
$fimgresult->execute(array('UserID'=>$fpro));
$frow = $fimgresult->fetch();
$fimgloc = $frow['Image'];
echo "<table class='proform'>";
echo "<tr><td><img src='$fimgloc' id='profileimgcrop' /></td><td id='profilehead'><h2>$fname $lname</h2><br />";
...
$getfavs->execute(array('UserID'=>$fpro));
