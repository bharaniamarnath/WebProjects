...
if(isset($_GET['pid'])){
$pictag = $_GET['pid'];
$picdet = $pdo->prepare("SELECT * FROM photodetails WHERE ID=:ID AND UserID=:UserID");
$picdet->execute(array('ID'=>$pictag, 'UserID'=>$suid));
...
echo "<th><h4 id='aboutpic'>Private Photo: $pename</h4></th>";
echo "<tr>";
echo "<td><img class='picscreen' src=$peimg /></td>";
echo "<input type='hidden' name='udpic' value='$peimg' />";
echo "</tr>";
echo "<tr>";
echo "<td id='aboutpic'><h4>$pedate<br />
$pedes</h4></td>";
echo "</tr>";
echo "<tr>";
echo "<td></td>";
echo "<td><input type='button' onclick=location.href='photos.php' id='photoviewbutton' value='Photos' /></td>";
echo "</tr>";
echo "</form>";
echo "</table><br />";
echo "</div>";
